...
A complete C compiler frontend supporting arrays, control structures, arithmetic expressions with parentheses, and detailed intermediate code generation.

## Features

- **Lexical Analysis**: Tokenizes C source code
- **Syntax Analysis**: Parses C grammar with support for:
...
- **Intermediate Code Generation**: Generates three-address code with detailed array operations
...
# Step 3: Compile
gcc -o compiler parser.tab.c
```

### Running:
```bash
# Test basic arithmetic with parentheses
Get-Content input1.c | ./compiler

# Test comprehensive features (arrays, loops, detailed IR)
Get-Content test_complex_ir.c | ./compiler
```
...
- `parser.y` - Parser grammar and semantic actions

### Test Files:
- `input1.c` - Basic test (arithmetic with parentheses)
- `test_complex_ir.c` - Comprehensive test (arrays, loops, detailed IR)
